...
function handlePushFrame(event) {
    ...
    for (const [id, contact] of contactState) {
        if (!updatedIds.has(id)) {
            const stationary = { ...contact, phase: BoardContactPhase.Stationary };
            contactState.set(id, stationary);
            currentContacts.push(stationary);
        }
    }
...
export const input = {
    ...
    subscribe(callback) {
        if (typeof window === "undefined" || !window.boardTouch) {
            throw new Error("Board touch bridge not available. Are you running inside a Board WebView?");
        }
